...
#####Move Card/Create Trigger#####
    ...
    ## This will cover all trigger initiations and activations ##
        ...
        if stackClass == 'cast': ## If the card is being cast, move it to the table so the stack markers can be added
            ...
            if (stackData['cost'] > 0 and stackData['costres'] != None) or (stackData['cost'] == 0 and stackData['res'] != None):
        ...
        if (not card in alignIgnore
            ...
            if not dictname in carddict:
                ...
                if not card.isFaceUp:
                    ...
                    index = 5
                elif re.search(r"单位", card.Type):
                    index = 0
                elif re.search(r"战器", card.Type):
                    index = 1
                elif re.search(r"阵略", card.Type):
                    index = 2
                else:
                    index = 7
